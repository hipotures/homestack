...
One scrollable checkbox tree contains all groups. At 110 columns or wider, the read-only details pane sits beside the tree; narrower terminals stack it below. Both panes scroll independently, while the target (including IP), selection counts, keyboard hints and Review/Cancel controls remain visible. Repository branches start collapsed, keep their expansion state on selection changes, and show group selection counts. Repository labels omit the configured owner only when it is redundant; the details pane retains the owner, checkout path and creation/last-push timestamps. Stable IDs and catalog mappings are unchanged; the shortened header catalog ID is available in full under the root's full details.

Up/Down moves through the tree, Left/Right collapses/expands, Space toggles an item or branch, and `0` selects the current branch. Clicking a checkbox toggles it; clicking a name only highlights it and displays its details. Clicking the disclosure arrow only expands/collapses the branch. Parent checkboxes reflect empty, full or partial selection. Mouse and keyboard toggles share the same availability, busy-state and filtering checks. `/` focuses the filter; bulk selection with a filter affects **visible matches only**. Hidden selections remain counted and appear in review.

Tab and Shift+Tab navigate the filter, tree, details (only when scrolling is needed), and action buttons. With the details pane focused, Up/Down and PageUp/PageDown scroll it without changing the tree selection. The mouse wheel scrolls the pane under the pointer, regardless of keyboard focus. Changing the highlighted item resets details to the top. `D` or Enter opens full details; Enter on the tree never applies. Escape closes details/review, clears an active filter, or cancels selection.
